...
#
**/
float   VERSION = 4.2;    // Beta 18 October May 2020
integer RSTATE = -1;      // RSTATE = 0 for release, 0 for beta, -1 for Release candidate
...
float WOOD_TIMES = 4.0;                     //  WOOD_TIMES=4                               how many times to give wood during the growing phase
string WOOD_OBJECT = "SF Wood";             //  WOOD_OBJECT=SF Wood                        What to rez as 'wood' product
...
string SF_WATER_TOWER = "SF Water Tower";   //  WELL=SF Water Tower                        Where to get water in auto water mode
string SF_WATER="SF Water";                 //  WATER_OBJECT=SF Water                      Name of water object to accept for manual watering
string SF_MANURE="SF Manure";               //  MANURE_OBJECT=SF Manure                    Name of object to accept as manure
...
save2Desc()
{
    llSetObjectDesc("T;"+PRODUCT_NAME+";"+status+";"+(string)(statusLeft)+";"+(string)llRound(water)+";"+(string)llRound(wood)+";"+plant+";"+(string)chan(llGetKey())+";"+(string)autoWater + ";"+languageCode);
}
...
setFoliage(integer visible, vector colour)
{
    ...
    if (visible == FALSE)
    {
        if (plantPrims == FALSE)
        {
            llSetLinkTexture(getLinkNum("Foliage"), TEXTURE_TRANSPARENT, ALL_SIDES);
        }
        else
        {
            list primNames = [];
    ...
    else
    {
        if (plantPrims == FALSE)
        {
            ...
            {
                llSetLinkTexture(getLinkNum("Foliage"), texture, ALL_SIDES);
            }
            llSetLinkColor(getLinkNum("Foliage"), colour, ALL_SIDES);
        }
        else
        {
            list primNames = [];
...
default
{
    ...
    touch_start(integer n)
    {
        ...
        if (HAS_WOOD && wood>=100.) opts += TXT_GET_WOOD;
